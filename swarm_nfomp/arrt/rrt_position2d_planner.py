...
class Position2DPlannerPath(Path[Position2D]):
    ...
    def add_intermediate_positions(self, total_positions_in_path: int) -> None:
        ...
        for seg in self.segments:
            # print(f"{self.length=}, {seg.length=}, {seg=}")
            number_of_positions = int(np.round(total_positions_in_path * seg.length / self.length))
            ...
            positions_on_segment = np.linspace(seg.start.as_vec(), seg.end.as_vec(), number_of_positions, endpoint=False)
            new_positions.extend([Position2D.from_vec(pos) for pos in positions_on_segment])
